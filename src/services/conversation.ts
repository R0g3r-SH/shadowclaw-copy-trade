...
const AGENT_TOOLS: Tool[] = [
  {
    ...
    description: 'Balance actual de la wallet del bot en Arbitrum: USDC (capital de trading) + ETH (solo para gas). Siempre reporta USDC como el capital disponible para operar.',
  ...
  {
    name: 'get_open_positions_live',
    description: 'Posiciones abiertas ahora mismo con P&L irealizado en tiempo real, precio actual de mercado, estado del trailing stop (pico, floor, distancia) y cambio de precio en 1h. Úsalo cuando pregunten cómo van las posiciones, cuánto ganamos/perdemos, o el estado de una posición específica.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'ask_portfolio_agent',
    description: 'Consulta al agente especialista en portafolio para preguntas complejas sobre historial de trades, análisis de performance, posiciones cerradas, P&L realizado. Para ver posiciones ABIERTAS con precio live usa get_open_positions_live.',
    input_schema: {
      type: 'object',
      properties: {
...
const publicClient = createPublicClient({ chain: arbitrum, transport: http(config.blockchain.alchemy.httpRpcUrl) });
...
export class ConversationService {
  ...
  private onWalletChanged: (() => Promise<void>) | null = null;
  ...
  setOnWalletChanged(cb: () => Promise<void>): void {
    this.onWalletChanged = cb;
  }
...
## REGLAS DE TRADING ACTIVAS (para diagnóstico y explicación):

GESTIÓN DE POSICIONES:
- Stop-loss fijo: -10%
- Take-profit fijo: +80%
- Trailing stop: se activa cuando la posición llega a +12% de ganancia. A partir de ahí, si el precio cae 7% desde el pico, se vende automáticamente. Ejemplo: posición llega a +40% → trailing floor en +33% → si cae a +33% se vende.
- El pico (peak_pnl_pct) se persiste en DB — sobrevive reinicios del bot.

SLIPPAGE DINÁMICO (basado en risk_score del token):
- Token estable (risk < 30): 1%
- Token establecido (risk 30-50): 2%
- Token volátil (risk 50-70): 10%
- Meme coin (risk > 70): 20%

SIZING DE POSICIÓN (fórmula del agente de trading):
- base = max_safe_position (2% del balance USDC)
- factor_wallet = wallet_score / 100
- factor_seguridad = 1 - (risk_score / 150)
- monto = base × factor_wallet × factor_seguridad

REGLAS DE SKIP INMEDIATO:
- Honeypot detectado
- risk_score ≥ 70
- Liquidez < $25,000
- Token creado hace < 1 hora
- Token ya subió > 50% en 1h (llegamos tarde)

DISCOVERY (wallets):
- 2 fuentes: (1) Arbiscan — 10 DEXes monitoreados, (2) Smart money — tokens con +40% en 24h → quién los compró 12-24h antes del pump
- Performance review cada 6h: ajusta scores por win rate real. Retira wallets con score ≤ 38
- Wallets orgánicas: detectadas del mempool con ≥ 15 swaps observados

ARQUITECTURA TÉCNICA:
- WebSocket Arbitrum vía Alchemy (watchBlocks, ~250ms/bloque) — NO usa webhooks
- Decodifica: Uniswap V3, SushiSwap, 1inch, Camelot, Paraswap, Odos, Balancer, TraderJoe, Ramses + multicall
- Aprobación ERC20: MaxUint256 una sola vez por token — no re-aprueba en cada venta
- Circuit breaker: pausa trading si pérdida diaria > límite en USD (basado en % del portfolio)

PROHIBIDO DECIR:
- "Sin webhook" — usa WebSocket, no webhooks
- "Activa polling" — el WebSocket ya ES el polling

## CUÁNDO USAR CADA TOOL:
- Posiciones abiertas con precio actual, P&L irealizado, trailing stop → get_open_positions_live
- Historial de trades, análisis complejo de performance → ask_portfolio_agent
- Wallets trackeadas, próximo discovery, smart money → ask_discovery_agent
- Estado general, circuit breaker → get_status
- Logs y errores recientes → get_recent_logs
- Uso de LLM y costos → get_llm_stats
- "Cómo vas" / "qué pasó" → get_status + get_open_positions_live + ask_discovery_agent

Personalidad:
- Español, conciso, directo, emojis
- Integra respuestas de sub-agentes naturalmente
- Siempre confirma acciones ejecutadas`;
  ...
  private async executeTool(name: string, args: any): Promise<any> {
    ...
    switch (name) {
      ...
      case 'get_open_positions_live':     return { positions: await this.portfolioAgent.getLivePositions() };
  ...
  private async getWalletBalance(): Promise<any> {
    try {
      const [balanceWei, usdcRaw, ethPrice] = await Promise.all([
        publicClient.getBalance({ address: config.wallet.address as `0x${string}` }),
        publicClient.readContract({ address: USDC_ADDRESS, abi: ERC20_BALANCE_ABI, functionName: 'balanceOf', args: [config.wallet.address as `0x${string}`] }),
        getEthPriceUsd(),
      ]);

      const ethBalance  = parseFloat(formatUnits(balanceWei, 18));
      const usdcBalance = parseFloat(formatUnits(usdcRaw as bigint, 6));
      const ethUsd      = ethBalance * ethPrice;
      const totalUsd    = ethUsd + usdcBalance;
      const positionSizeUsd = usdcBalance * config.trading.positionSizePct;
      ...
      return {
        ...
        nota: 'ETH es solo para gas. USDC es el capital de trading.',
      };
    } catch (error: any) {
  ...
  private async getPnl(days: number): Promise<any> {
    ...
    const result = await this.db.query(
      `SELECT COUNT(*) as total_trades,
       ...
       FROM copied_trades
       WHERE status = 'closed' AND closed_at >= NOW() - make_interval(days => $1)`,
      [safeDays]
    );
